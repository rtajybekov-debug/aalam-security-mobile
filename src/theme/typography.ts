/** Style guide: use these tokens instead of hardcoded fontSize/fontWeight */
export const typography = {
  display: { fontSize: 32, lineHeight: 38, fontWeight: "700" as const },
  heading: { fontSize: 24, lineHeight: 30, fontWeight: "700" as const },
  headingBold: { fontSize: 24, lineHeight: 30, fontWeight: "800" as const },
  title: { fontSize: 18, lineHeight: 24, fontWeight: "600" as const },
  subtitle: { fontSize: 16, lineHeight: 22, fontWeight: "600" as const },
  body: { fontSize: 16, lineHeight: 22, fontWeight: "400" as const },
  bodyBold: { fontSize: 16, lineHeight: 22, fontWeight: "600" as const },
  meta: { fontSize: 14, lineHeight: 20, fontWeight: "500" as const },
  caption: { fontSize: 12, lineHeight: 16, fontWeight: "500" as const },
  captionBold: { fontSize: 12, lineHeight: 16, fontWeight: "600" as const },
  label: { fontSize: 11, lineHeight: 14, fontWeight: "600" as const },
} as const;
